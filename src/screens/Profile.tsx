  ...
  const submitUpdate = (values: Object) => {
    ...
    fetch('https://api.entegre.pro/ui/UIntegration/UpdateUser', {
      ...
      .then(res => {
        console.log(res)
        setIsLoading(false)
        Alert.alert(res.status ? 'Güncelleme Başarılı' : 'Hata', res.message, [
          {
            text: 'Tamam',
                          ...
                          {pastOrders.orderLines &&
                            pastOrders.orderLines
                              ...
                              .map((orderLine: any, indexLine) => (
                                <View style={styles.product} key={indexLine}>
                                  ...
                                  <Text style={styles.productPrice}>
                                    {(orderLine.price + pastOrders.variants.filter((variant: any) => variant.orderID === order.orderID && variant.lineID === orderLine.lineID).reduce((total, curr: any) => total += curr.price, 0)) * orderLine.qty.toFixed(1)} ₺
                                  </Text>
                                </View>
                              ))}
